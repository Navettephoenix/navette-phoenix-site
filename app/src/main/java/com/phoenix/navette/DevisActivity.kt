...
class DevisActivity : AppCompatActivity() {
    ...
    private fun setupDropdowns() = with(binding) {
        ...
        val passengerAdapter = ArrayAdapter(this@DevisActivity, android.R.layout.simple_list_item_1, passengers)
        ...
        val optionsAdapter = ArrayAdapter(this@DevisActivity, android.R.layout.simple_list_item_1, options)
        autoOption.setAdapter(optionsAdapter)
        autoOption.setText(options.first(), false)
        autoOption.setOnClickListener { autoOption.showDropDown() }
        autoOption.setOnFocusChangeListener { _, hasFocus -> if (hasFocus) autoOption.showDropDown() }
    }
    ...
    private fun setupActions() = with(binding) {
        buttonMap.setOnClickListener {
            ...
            if (destination.isBlank()) {
                Toast.makeText(this@DevisActivity, R.string.message_map_destination_required, Toast.LENGTH_SHORT).show()
            } else {
    ...
    private fun submitForm() {
        ...
        val option = binding.autoOption.text?.toString()?.trim().orEmpty()

        if (!validate(name, phone, email, depart, destination)) {
            return
        }
        ...
        NavetteApi.post("devis_hors_secteur", fields) { success, error ->
            runOnUiThread {
                setLoading(false)
                if (success) {
